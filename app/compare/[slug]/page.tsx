...
export async function generateMetadata({ params }: ComparisonPageProps): Promise<Metadata> {
  ...
  const [player1Slug, player2Slug] = slug.replace('vs-', '-vs-').split('-vs-');

  const player1 = players.find(p => p.slug === player1Slug);
  const player2 = players.find(p => p.slug === player2Slug);

  if (!player1 || !player2) {
    return {
      title: 'Jämförelse ej hittad - Checkoutstrategi',
    };
  }

  return {
    title: `${player1.name} vs ${player2.name} - Jämförelse 2025 | Checkoutstrategi`,
    description: `Jämför ${player1.name} och ${player2.name} som checkout-leverantörer. Se priser, funktioner, integrationer och recensioner för att hitta rätt lösning för din e-handel.`,
    openGraph: {
      title: `${player1.name} vs ${player2.name} - Jämförelse`,
      description: `Detaljerad jämförelse av ${player1.name} och ${player2.name} för svenska e-handlare.`,
      type: 'article',
    },
    alternates: {
      canonical: `https://checkoutstrategi.se/compare/${slug}`,
    },
  };
}
...
export async function generateStaticParams() {
  ...
  return commonComparisons.map((slug) => ({
    slug,
  }));
}
